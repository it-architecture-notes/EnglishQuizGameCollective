...
void main() {
  testWidgets('real theme + real font an check', (tester) async {
    await loadInter();
    ...
    debugPrint('TILE "an" box (REAL theme+font): width=${(tileBox.right-tileBox.left).toStringAsFixed(3)} height=${(tileBox.bottom-tileBox.top).toStringAsFixed(3)}');
    ...
    debugPrint('DIALOG "an" box (REAL theme+font): width=${(dialogBox.right-dialogBox.left).toStringAsFixed(3)} height=${(dialogBox.bottom-dialogBox.top).toStringAsFixed(3)}');
  });
}
