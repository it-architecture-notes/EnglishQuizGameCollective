...
/// Remainder height fraction dedicated to the prompt card vs reply buttons (`line1 != null`).
const Map<QuestionLayoutTier, ({double prompt, double buttons})>
    _dialogueCompletionRemainderShare = {
...
/// Discrete gap and padding metrics per [QuestionLayoutTier].
typedef _DialogueCompletionGaps = ({
  double promptToButtonsGap,
  double containerHorizontalPadding,
  double buttonSpacing,
});

const Map<QuestionLayoutTier, _DialogueCompletionGaps> _dialogueCompletionGaps =
    {
...
/// Discrete prompt card padding and speaker metrics per [QuestionLayoutTier].
typedef _DialogueCompletionPromptPreset = ({
...
const Map<QuestionLayoutTier, _DialogueCompletionPromptPreset>
    _dialogueCompletionPromptPresets = {
  QuestionLayoutTier.phone16to9: (
    cardPaddingTop: 12.0,
...
/// Discrete button preset keyed by option count ($N=2, 3, 4$).
...
/// Map of option count to button preset per [QuestionLayoutTier].
/// Keys: 4 (4+ buttons), 3 (3 buttons), 2 (1-2 buttons).
const Map<QuestionLayoutTier, Map<int, _ButtonPreset>>
    _dialogueCompletionButtonPresets = {
  QuestionLayoutTier.phone16to9: {
    4: (
      fontSize: 14.0,
      singleLineHeight: 44.0,
      multiLineHeight: 56.0,
      horizontalPadding: 24.0
    ),
    3: (
      fontSize: 15.0,
      singleLineHeight: 48.0,
      multiLineHeight: 60.0,
      horizontalPadding: 24.0
    ),
    2: (
      fontSize: 16.0,
      singleLineHeight: 52.0,
      multiLineHeight: 64.0,
      horizontalPadding: 24.0
    ),
  },
  QuestionLayoutTier.phoneClassic2to1: {
  ...
  QuestionLayoutTier.phoneTransition: {
    4: (
      fontSize: 14.0,
      singleLineHeight: 45.0,
      multiLineHeight: 58.0,
      horizontalPadding: 24.0
    ),
    3: (
      fontSize: 15.0,
      singleLineHeight: 49.0,
      multiLineHeight: 62.0,
      horizontalPadding: 24.0
    ),
    2: (
      fontSize: 16.0,
      singleLineHeight: 53.0,
      multiLineHeight: 66.0,
      horizontalPadding: 24.0
    ),
  },
  QuestionLayoutTier.phoneFlagship: {
    ...
    2: (
      fontSize: 17.0,
      singleLineHeight: 56.0,
      multiLineHeight: 70.0,
      horizontalPadding: 28.0
    ),
  },
  QuestionLayoutTier.phoneSuperTall: {
    4: (
      fontSize: 15.0,
      singleLineHeight: 50.0,
      multiLineHeight: 62.0,
      horizontalPadding: 28.0
    ),
    3: (
      fontSize: 16.0,
      singleLineHeight: 54.0,
      multiLineHeight: 66.0,
      horizontalPadding: 28.0
    ),
    2: (
      fontSize: 17.0,
      singleLineHeight: 58.0,
      multiLineHeight: 72.0,
      horizontalPadding: 28.0
    ),
  },
  QuestionLayoutTier.phoneUltraTall: {
    ...
    3: (
      fontSize: 17.0,
      singleLineHeight: 56.0,
      multiLineHeight: 68.0,
      horizontalPadding: 28.0
    ),
    2: (
      fontSize: 18.0,
      singleLineHeight: 60.0,
      multiLineHeight: 74.0,
      horizontalPadding: 28.0
    ),
  },
  QuestionLayoutTier.tablet4to3: {
    ...
    2: (
      fontSize: 20.0,
      singleLineHeight: 64.0,
      multiLineHeight: 78.0,
      horizontalPadding: 32.0
    ),
  },
  QuestionLayoutTier.tablet3to2: {
    ...
    3: (
      fontSize: 20.0,
      singleLineHeight: 62.0,
      multiLineHeight: 76.0,
      horizontalPadding: 34.0
    ),
    2: (
      fontSize: 21.0,
      singleLineHeight: 66.0,
      multiLineHeight: 82.0,
      horizontalPadding: 34.0
    ),
  },
  QuestionLayoutTier.tablet16to10: {
    ...
    2: (
      fontSize: 21.0,
      singleLineHeight: 68.0,
      multiLineHeight: 84.0,
      horizontalPadding: 36.0
    ),
  },
  QuestionLayoutTier.tablet16to9: {
    ...
    3: (
      fontSize: 21.0,
      singleLineHeight: 66.0,
      multiLineHeight: 80.0,
      horizontalPadding: 36.0
    ),
    2: (
      fontSize: 22.0,
      singleLineHeight: 70.0,
      multiLineHeight: 86.0,
      horizontalPadding: 36.0
    ),
  },
};
...
_ButtonPreset _resolveButtonPreset(QuestionLayoutTier tier, int optionCount) {
  ...
  return _dialogueCompletionButtonPresets[tier]![countKey]!;
}
...
class _DialogueCompletionLayout {
  const _DialogueCompletionLayout({
...
/// `VideoConversationQuizBody`'s `DialogueCompletion` answer_type: the shared regular-question
/// media frame, a white answer panel pulled up over its bottom edge, [data.line1] as the bold
/// prompt, then the same [McqPillAnswerButton] controls. The media is a static [imagePath]
/// instead of a shared video controller.
...
class _DialogueCompletionQuizBodyState
  ...
  /// Discrete Presets & Cascading Extension Layout:
  /// - Reads exact gaps, prompt metrics, and button presets from per-tier tables.
  /// - Measures the live option set to find the tallest button, sharing that height across all choices.
  /// - Derives `line1FontSize = 1.1 × resolvedButtonPreset.fontSize` live.
  /// - Sums any button or prompt shortfall and extends upward over media in a `Stack`.
  _DialogueCompletionLayout _computeLayout({
    ...
    final gaps = _dialogueCompletionGaps[tier]!;
    final promptPreset = _dialogueCompletionPromptPresets[tier]!;
    final remainderSplit = _dialogueCompletionRemainderShare[tier]!;
    ...
    for (final opt in _options) {
      ...
      if (lines > maxOptionLines) {
        maxOptionLines = lines;
      }
    }
    ...
    if (maxOptionLines <= 1) {
      ...
      // Beyond-Cap: Add +20px per extra line beyond 2
      sharedButtonHeight =
    ...
    // Media is always rendered at its full, fixed `mediaHeight` — it is never
    // shrunk. Cascade extension is covered by drawing the prompt card on top
    // of (over) the media instead, up to the full media height; beyond that
    // the pathological scroll fallback engages. How much of the media ends
    // up covered is a content-authoring concern, not a hard code floor.
    final maxMediaExtension = hasImage ? mediaHeight : 0.0;
    ...
    // Accounting height only — how much of the container's reserved space is
    // *not* covered by the prompt card. The media widget itself always
    // renders at the full `mediaHeight` regardless of this value.
    final mediaVisibleHeight =
        hasImage ? max(0.0, mediaHeight - promptExtension) : 0.0;
    ...
    if (maxOptionLines > 2 || optionCount > 4) {
      debugPrint(
        '[QuestionLayout][DialogueCompletion][Warning] '
    ...
    if (overflowBeyondMedia > 0.0) {
      debugPrint(
        '[QuestionLayout][DialogueCompletion][Severe] '
    ...
    return _DialogueCompletionLayout(
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (context, bodyConstraints) {
        ...
        final gaps = _dialogueCompletionGaps[budget.tier]!;
        final promptPreset = _dialogueCompletionPromptPresets[budget.tier]!;
        ...
        if (_lastLayoutLogKey != layoutLogKey) {
          ...
          debugPrint(
            '[QuestionLayout][DialogueCompletion] '
            ...
            'image=$hasImage audio=$hasAudioButton',
          );
        }
            ...
            SizedBox(
              ...
              child: Stack(
                ...
                children: [
                  if (hasImage)
                    Positioned(
                      // Media always renders at its full, fixed `mediaHeight`
                      // — never shrunk. When the prompt card below grows
                      // past its nominal size, the container above is
                      // shorter than `mediaHeight + promptCardHeight`, so
                      // the card (painted after media, on top) covers the
                      // bottom portion of the media rather than the media
                      // itself shrinking.
                      top: 0,
                      left: 0,
                      right: 0,
