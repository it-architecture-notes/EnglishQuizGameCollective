...
`all-ai-common/rules/rules.md` first (plan before implement unless user
says fix/apply).
...
| Prior words by type (optional) | `all-ai-common/output/prior-words-by-type.md` |
| Distractor / template rules | `all-ai-common/skills/audit-quiz-level/SKILL.md` |
...
1. Open each file in `all-ai-common/references/final words/`:
...
2. For each later level, read `translations.json` (`english_word` entries) and
   ...
   Or use `all-ai-common/output/prior-words-by-type.md` to see which
