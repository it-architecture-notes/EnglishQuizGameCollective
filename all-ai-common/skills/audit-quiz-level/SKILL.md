...
Audit one quiz level at a time. Read project rules in `all-ai-common/rules/rules.md` first (one issue at a time, plan before implement unless user says fix/apply).
...
| Oxford 3000 word list | `all-ai-common/references/remove-word-list-references/3000 words oxford.txt` |
| LanGeek common nouns | `all-ai-common/references/remove-word-list-references/langeek-500-most-common-nouns.txt` |
...
3. **Run the prior-usage script** (do not hand-scan prior level JSON for consumed words):
   ...
   - Save JSON to `all-ai-common/output/{level-id}-prior-words.json` when the audit will continue across turns.
...
Default: **report inline in chat**, wait for approval before editing JSON (per project rules). Write to `all-ai-common/output/{level-id}-audit.md` only if explicitly requested.
...
For **focused vocabulary improvement** (unused reference words, misplaced later-level words, full swap proposals with question edits), see **`all-ai-common/skills/improve-level-vocabulary/SKILL.md`**.
