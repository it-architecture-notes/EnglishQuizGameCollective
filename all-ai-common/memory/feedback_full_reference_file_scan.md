...
When proposing vocabulary swaps or new-word candidates for a level (via `audit-quiz-level` or `improve-level-vocabulary`), cross-check candidates against **all 8** files in `all-ai-common/references/final words/`, not just `verbs-400.csv`, `adjectives-300.csv`, `adverbs-150.csv`:
