...
The `audit-quiz-level` skill's grammar-progression table (`all-ai-common/skills/audit-quiz-level/SKILL.md`) no longer gates grammar level-by-level across all 12 mainLevels. It's now 4 macro-bands ("Parts"), and everything aggregated into a Part is usable at **any** ML within that Part's range, not just from the ML that originally "introduced" it.
