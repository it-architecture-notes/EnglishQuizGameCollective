...
## Key Context Files
- `all-ai-common/context/active-progress-context.md` — current issue (currently: Issue-27 translations page spec, story overlay config spec, and misc branch fixes — no cleared/new active issue since e07ac8d)
- `all-ai-common/context/progress-context-archive.md` — completed issues summary
- `all-ai-common/context/architecture-technical-context.md` — tech/asset layout
- `all-ai-common/context/story-templates-and-design.md` — story template registry (now 5 templates, see Current State)
- `all-ai-common/context/page-designs-and-templates.md` — per-quiz-template UX/JSON schema reference
- `all-ai-common/rules/rules.md` — project rules
...
- Context files live in `all-ai-common/` (shared by all AI agents)
...
## Architecture Notes
- [Kids/adults flavor + tutorial system](project_app_flavor_and_tutorial.md) — AppConfig.flavor split, flavor-specific flow/achievements/quiz-content JSON, `widgets/tutorial/` overlay framework. Not yet documented in `all-ai-common/context/`.
...
## Workflow Shorthand
- ["r-r" = "read and respond in pipe"](feedback_rr_shorthand.md) — user shorthand for checking the multi-agent Message Pipeline and posting a reply there.

## Audit Workflow
- Prior consumed words file: `all-ai-common/output/prior-words-by-type.md` — read this instead of running the gather script. Check the header to confirm it matches the target level; regenerate with the script only if auditing a different level.
