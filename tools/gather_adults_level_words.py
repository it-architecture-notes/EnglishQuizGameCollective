#!/usr/bin/env python3
"""
Gather all English vocabulary words from each adult quiz level under their level titles.
...
Features:
- Orders levels according to `game-flow.json` (with any remaining unmapped disk levels appended).
...
Usage examples:
  python3 tools/gather_adults_level_words.py
  python3 tools/gather_adults_level_words.py --format markdown -o all-ai-common/output/adults-vocab-by-level.md
  python3 tools/gather_adults_level_words.py --word-index
  python3 tools/gather_adults_level_words.py --duplicates-only
  python3 tools/gather_adults_level_words.py --search "ready"
  python3 tools/gather_adults_level_words.py --format json
"""
...
def gather_all_adult_levels(root: Path) -> list[LevelVocabulary]:
    ...
    flow_path = root / "app" / "assets" / "data" / "flow" / "game-flow.json"

    flow_entries = load_game_flow(flow_path)
    processed_dirs: set[str] = set()
    levels: list[LevelVocabulary] = []

    # 1. Process levels defined in game-flow.json in order
    order_idx = 1
    for entry in flow_entries:
        ...
        level_adult_dir = levels_root / disk_name / "adults"
    ...
    # 2. Collect any remaining disk levels not mentioned in game-flow.json
...
def format_text_level_view(levels: list[LevelVocabulary], search_filter: str | None = None) -> str:
    lines: list[str] = []
    lines.append("=" * 80)
    lines.append("  ENGLISH QUIZ GAME — ADULT LEVEL VOCABULARY BY LEVEL TITLE")
    lines.append("=" * 80)
...
def format_markdown_level_view(levels: list[LevelVocabulary], search_filter: str | None = None) -> str:
    lines: list[str] = []
    lines.append("# Adult Quiz Levels — Vocabulary by Level Title\n")
...
def format_inverted_index(
    ...
    if as_markdown:
        lines.append("# Adult Vocabulary — Inverted Word Index (Word → Levels)\n")
        if duplicates_only:
        ...
        lines.append("=" * 80)
        lines.append("  INVERTED VOCABULARY INDEX (Word -> Levels Where Used)")
        if duplicates_only:
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Gather all vocabulary words from each adult quiz level under level titles."
    )
    parser.add_argument(
        "--format",
        "-f",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Output file path (default: all-ai-common/output/adults-words-by-level.<ext>)",
    )
    parser.add_argument(
        "--stdout",
        action="store_true",
    ...
    if args.duplicates_only or args.word_index:
        ...
        default_filename = f"adults-word-index.{ext}"
    else:
        if is_md:
        ...
        default_filename = f"adults-words-by-level.{ext}"
    ...
    if args.stdout:
        ...
        total_wp = sum(len(l.wordpairs) for l in levels)
        print("=" * 80)
        print("  ✅ ADULT VOCABULARY GATHERING COMPLETE")
        print("=" * 80)
